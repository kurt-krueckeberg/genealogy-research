= Understanding the Rootsmagic Database

== RootsMagic Sqlite3 DB
RootsMagic8 (RM8)  is Sqlite3 relational database that uses a custom collation scheme RMNOCASE that prevents `Order By` from working.

== SQLite3 Tools

In addition to the CLI Ubuntu *sqlite3* package, this is list of https://sqlitetoolsforrootsmagic.com[SQLite Tools] that can be used with RootsMagic or any Sqlite3 db. 
