...
and memory low. However, this causes the table not to be in normal form because the same birth and date years are repeated in every row with the same `OwerID`. To confirm this is the case, consider these two queries 
...
The first select rows for person's with more than one name assigned to them. The second query selects rows where any of there attributes OwnerID, BirthYear or DeathYear diifer. Both queries return the exactly the same 
...
Other self-descriptiive attributes are `Prefix`, `Suffix` and `Nickname`. `isPrimary` is (an int) that has this set of values and associated meanings:
