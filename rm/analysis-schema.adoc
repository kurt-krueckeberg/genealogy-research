...
`ChildTable` has key `RecID`, child identifier `ChildID`, a foreign key referencing the `PersonTable.PersonID`, and `FamilyID`, a foreign key referencing `FamilyTable.FamilyID`. QUESTION: Can it be zero, meaning the family is not yet known?
