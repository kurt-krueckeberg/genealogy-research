...
`PersonTable.PersonID` key identifies a unique person (in the family tree) with sex of `Sex` and with unique `UniqueID` (which apparently is a `GUID` or hash value that is **GEDCOM** standard field).
