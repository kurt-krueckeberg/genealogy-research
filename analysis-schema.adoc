...
==== Predicate
The `PersonTable` `PersonID` key identifies a unique individual (in the family tree) with sex `Sex`, unique `UniqueID` (that apparently is a `GUID` or hash value that is a **GEDCOM** standard field),
...
- `Sex` is acutally a  boolean but is defined as `integer`.
- `Living` is acutally a  boolean but is defined as `integer`.
- Since `ParentID == 0` and `SpouseID == 0` for most rows, and they are not foreign keys referencing `PersonTable.PersonID`.
...
`OwnerID` is a foreign key corresponding to the `PersonTable.PersonID`, the person to whom the name belongs. We infer this from the fact that:
