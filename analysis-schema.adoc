...
- `Sex` is acutally a boolean but is defined as `integer`. `0` is male and `1` is female.
- `SpouseID`
   Spouse Identification Number, linking to of `FamilyTable.FamilyID`, (0 if no spouse) [MRIN with one of spouses, possibly last active in Pedigree view or when selected via Spouses Status Bar, others?]
- `Living` is acutally a boolean 
